			...
			{isVisible &&
				createPortal(
					...
					>
						<div className="flex items-center gap-3 mb-3 border-b border-ds-border pb-3">
							...
							<div>
								...
								<div className="text-xs text-ds-text-muted font-mono">
									{player.team} • {getPositionName(player.position)}
								</div>
							</div>
						...
						{loading ? (
							<div className="py-4 text-center text-ds-text-muted text-sm font-mono animate-pulse">
								Loading stats...
							</div>
						) : summary ? (
								...
								<div>
									...
									<div className="grid grid-cols-5 gap-1">
										{summary.history
											.slice()
											.reverse()
											.slice(historyOffset, historyOffset + 5)
											.map((fixture) => {
												...
												return (
													...
													>
														...
														<span
															className={`text-sm font-bold ${fixture.total_points >= 6 ? 'text-ds-accent' : fixture.total_points >= 3 ? 'text-ds-text' : 'text-ds-text-muted'}`}
														>
															{fixture.total_points}
														</span>
														<span className="text-[9px] text-ds-text-muted/70">
															{fixture.opponent_short_name} ({fixture.was_home ? 'H' : 'A'})
														</span>

														{/* Tooltip */}
														<div
															className={`absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-48 bg-ds-card border border-ds-border shadow-xl rounded-lg p-3 z-50 ${isActive ? 'block' : 'hidden md:group-hover:block'} pointer-events-none`}
														>
															<div className="text-xs font-bold text-ds-text border-b border-ds-border pb-1 mb-2 whitespace-nowrap flex justify-between">
																<span>
																	vs {fixture.opponent_short_name} ({fixture.was_home ? 'H' : 'A'})
																</span>
																<span className={resultColor}>
																	{resultChar} {teamScore}-{oppScore}
																</span>
															</div>
															<div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-ds-text-muted">
																<div className="flex justify-between">
																	<span>Points:</span>{' '}
																	<span className="text-ds-primary font-bold">{fixture.total_points}</span>
																</div>
																<div className="flex justify-between">
																	<span>Mins:</span> <span>{fixture.minutes}</span>
																</div>
																{fixture.goals_scored > 0 && (
																	<div className="flex justify-between">
																		<span className="text-green-400">Goals:</span>{' '}
																		<span className="font-bold text-ds-text">{fixture.goals_scored}</span>
																	</div>
																)}
																{fixture.assists > 0 && (
																	<div className="flex justify-between">
																		<span className="text-blue-400">Assists:</span>{' '}
																		<span className="font-bold text-ds-text">{fixture.assists}</span>
																	</div>
																)}
																{fixture.bonus > 0 && (
																	<div className="flex justify-between">
																		<span className="text-yellow-400">Bonus:</span>{' '}
																		<span className="font-bold text-ds-text">{fixture.bonus}</span>
																	</div>
																)}
																{fixture.saves > 0 && (
																	<div className="flex justify-between">
																		<span className="text-orange-400">Saves:</span>{' '}
																		<span className="font-bold text-ds-text">{fixture.saves}</span>
																	</div>
																)}
																<div className="flex justify-between">
																	<span>BPS:</span> <span>{fixture.bps}</span>
																</div>
																{fixture.expected_goals && (
																	<div className="flex justify-between">
																		<span>xG:</span> <span>{fixture.expected_goals}</span>
																	</div>
																)}
																{fixture.expected_assists && (
																	<div className="flex justify-between">
																		<span>xA:</span> <span>{fixture.expected_assists}</span>
																	</div>
																)}
															</div>
														</div>
											...
											{summary.history_vs_opponent.slice(0, 5).map((fixture) => {
												...
												return (
													<div
														...
														className="group relative flex flex-col items-center bg-ds-bg/50 rounded p-1 border border-ds-border hover:bg-ds-surface transition-colors cursor-help"
													>
														<span className="text-[8px] font-mono text-ds-text-muted">
															{fixture.season.split('-')[1] || fixture.season}
														</span>
														<span
															className={`text-sm font-bold ${fixture.points >= 6 ? 'text-ds-accent' : fixture.points >= 3 ? 'text-ds-text' : 'text-ds-text-muted'}`}
														>
															{fixture.points}
														</span>
														...
														>
															<div className="text-xs font-bold text-ds-text border-b border-ds-border pb-1 mb-2 whitespace-nowrap">
																...
																<span className="ml-2 text-ds-text-muted font-normal">
																	GW{fixture.gameweek} ({fixture.was_home ? 'H' : 'A'})
																</span>
															</div>
															<div className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs text-ds-text-muted">
																<div className="flex justify-between">
																	<span>Points:</span>{' '}
																	<span className="text-ds-primary font-bold">{fixture.points}</span>
																</div>
																<div className="flex justify-between">
																	<span>Mins:</span> <span>{fixture.minutes}</span>
																</div>
								...
								<div>
									...
									<div className="space-y-1">
										{summary.fixtures.slice(0, 3).map((fixture) => (
											<div
												key={fixture.id}
												className="flex justify-between items-center text-xs bg-ds-bg/30 p-1.5 rounded border border-ds-border/50"
											>
												<span className="font-mono text-ds-text-muted">GW{fixture.event}</span>
												<span className="font-bold">
													{fixture.is_home ? '(H)' : '(A)'} vs{' '}
													{fixture.is_home ? fixture.team_a_short : fixture.team_h_short}
												</span>
												<span
													className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${getFdrBadgeClass(fixture.difficulty)}`}
												>
