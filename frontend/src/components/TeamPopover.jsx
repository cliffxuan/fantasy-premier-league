						...
						<div className="flex items-center gap-3 mb-4 border-b border-ds-border pb-3">
							...
							{team.position && (
								<div className="ml-auto text-right">
									<div
										className={`text-xl font-bold ${team.position <= 4 ? 'text-ds-primary' : team.position >= 18 ? 'text-ds-danger' : 'text-ds-text'}`}
									>
										#{team.position}
									</div>
										...
										{summary.recent_results && summary.recent_results.length > 0 ? (
											summary.recent_results.slice(historyOffset, historyOffset + 5).map((fixture) => {
												...
												return (
													...
													>
														<span
															className={`text-[10px] font-bold ${fixture.result === 'W' ? 'text-ds-accent' : fixture.result === 'L' ? 'text-ds-danger' : 'text-ds-text-muted'}`}
														>
															{fixture.result}
														</span>
														...
														<span className="text-[8px] text-ds-text-muted/70 truncate w-full text-center mt-0.5">
															...
															<span className="text-[8px] text-ds-text-muted/50 font-mono">
																({fixture.is_home ? 'H' : 'A'})
															</span>
														</span>
														...
														{/* Tooltip */}
														<div
															className={`absolute bottom-full mb-2 left-1/2 -translate-x-1/2 w-48 bg-ds-card border border-ds-border shadow-xl rounded-lg p-3 z-50 ${isActive ? 'block' : 'hidden md:group-hover:block'} pointer-events-none text-left`}
														>
															<div className="text-xs font-bold text-ds-text border-b border-ds-border pb-1 mb-2 whitespace-nowrap flex justify-between">
																<span>
																	vs {fixture.opponent_short} ({fixture.is_home ? 'H' : 'A'})
																</span>
																<span
																	className={
																		fixture.result === 'W'
									...
									<div className="grid grid-cols-1 gap-1">
										{summary.top_players &&
											summary.top_players.slice(0, 3).map((player) => (
												<div
													key={player.id}
													className="flex items-center gap-2 p-1 rounded hover:bg-ds-card-hover transition-colors"
												>
													...
													<div className="flex-1 min-w-0">
														...
														<div className="text-[10px] text-ds-text-muted">
															{getPositionName(player.element_type)} • £{player.cost}m
														</div>
													</div>
