...
const PitchView = ({ players, renderCard, className = '' }) => {
	...
	return (
		<div className={`bg-ds-card rounded-xl p-2 md:p-8 relative border border-ds-border min-h-[500px] md:min-h-[600px] flex flex-col justify-between overflow-hidden ${className}`}>
			...
			<div className="flex justify-center gap-1 md:gap-4 z-10">
				{gkp.map((p, i) => <React.Fragment key={p.id || `gkp-${i}`}>{renderCard(p, i)}</React.Fragment>)}
			</div>
			<div className="flex justify-center gap-1 md:gap-4 z-10">
				{def.map((p, i) => <React.Fragment key={p.id || `def-${i}`}>{renderCard(p, i)}</React.Fragment>)}
			</div>
			<div className="flex justify-center gap-1 md:gap-4 z-10">
				{mid.map((p, i) => <React.Fragment key={p.id || `mid-${i}`}>{renderCard(p, i)}</React.Fragment>)}
			</div>
			<div className="flex justify-center gap-1 md:gap-4 z-10">
				{fwd.map((p, i) => <React.Fragment key={p.id || `fwd-${i}`}>{renderCard(p, i)}</React.Fragment>)}
			</div>
		</div>
	);
};
