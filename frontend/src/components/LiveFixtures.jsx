...
const LiveFixtures = () => {
	...
	const FixtureRow = ({ f }) => {
		...
		return (
			<div className={`flex items-center justify-between p-3 md:p-4 rounded-lg transform transition-all hover:scale-[1.01] ${isLive ? 'bg-ds-card border-l-4 border-l-green-500 border-y border-r border-ds-border shadow-md' : 'bg-ds-card/50 border border-ds-border'}`}>
				...
				<div className="flex flex-col items-center justify-center min-w-[70px] md:min-w-[90px] px-2">
					{f.started ? (
						<div className="flex flex-col items-center">
							<div className={`flex items-center gap-1 font-mono text-xl md:text-2xl font-bold ${isLive ? 'text-white' : 'text-ds-text-muted'}`}>
								<span>{f.team_h_score}</span>
								<span className="text-ds-text-muted/50">-</span>
								<span>{f.team_a_score}</span>
							</div>
							{isLive && (
								<span className="text-[10px] text-green-400 font-bold animate-pulse mt-1">{f.minutes}'</span>
							)}
							{f.finished_provisional && (
								<span className="text-[10px] text-ds-text-muted font-bold mt-1">FT</span>
							)}
						</div>
					) : (
						<div className="flex flex-col items-center">
	...
	return (
		<div className="bg-ds-card/30 rounded-xl p-4 md:p-6 border border-ds-border/50">
			<h3 className="text-xl font-bold mb-6 flex items-center gap-3 text-ds-text">
				...
				<span className="text-xs font-normal text-ds-text-muted bg-ds-surface px-2 py-1 rounded-full border border-ds-border/50">LIVE</span>
			</h3>

			<div className="grid grid-cols-1 xl:grid-cols-2 gap-3 md:gap-4">
				{sortedFixtures.map(f => <FixtureRow key={f.id} f={f} />)}
			</div>

			{fixtures.length === 0 && <div className="text-ds-text-muted text-center py-8">No specific fixtures found for this Gameweek.</div>}
		</div>
	);
};
