		...
		<div className="min-h-screen flex flex-col bg-ds-bg text-ds-text font-sans selection:bg-ds-primary selection:text-white">
			...
			<div className="sticky top-0 z-50 bg-ds-bg/95 backdrop-blur-md border-b border-ds-border transition-all duration-300">
				...
				<div className="hidden md:block w-full max-w-[1400px] mx-auto px-4 md:px-8 mt-2">
					<nav ref={tabsContainerRef} className="flex overflow-x-auto gap-6 custom-scrollbar -mb-[1px]">
						{TABS.map((tab) => (
							<button
								key={tab.id}
								ref={(el) => (tabRefs.current[tab.id] = el)}
								onClick={() => handleTabChange(tab.id)}
								className={`pb-3 px-1 font-bold text-sm md:text-base whitespace-nowrap transition-all border-b-2 ${
									activeTab === tab.id
			...
			{/* Main Content */}
			<main className="w-full max-w-[1400px] mx-auto p-4 md:p-8 box-border flex-1 pb-24 md:pb-8">
			...
			<div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-ds-bg/95 backdrop-blur-md border-t border-ds-border transition-all duration-300">
				<div className="w-full max-w-[1400px] mx-auto px-4 md:px-8">
					<nav className="flex overflow-x-auto gap-6 custom-scrollbar -mb-[1px]">
						{TABS.map((tab) => (
							<button
								key={tab.id}
								onClick={() => handleTabChange(tab.id)}
								className={`py-4 px-1 font-bold text-sm md:text-base whitespace-nowrap transition-all border-t-4 ${
									activeTab === tab.id
					...
					<div className="bg-ds-card w-full max-w-3xl rounded-xl border border-ds-border shadow-2xl flex flex-col max-h-[85vh] min-h-[500px] animate-in zoom-in-95 duration-200">
						...
						<div className="p-6 border-t border-ds-border bg-ds-card/50 flex justify-end gap-3">
							...
							<button
								...
								className={`flex items-center gap-2 px-4 py-2 rounded-md font-bold text-sm transition-all shadow-lg ${
									copySuccess
